...
class Verticalimagescrool extends StatelessWidget {
  const Verticalimagescrool({
    ...
    required this.targetPage, // Page to navigate to
  });
  ...
  final Widget targetPage; // Target page to navigate to

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        // Navigate to targetPage when the icon is clicked
        if (onTap != null) {
          onTap!();
        } else {
          // If onTap is not provided, use Get.to() for navigation
          Get.to(() => targetPage);
        }
      },
      child: Padding(
